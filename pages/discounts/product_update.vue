<template>
	<view class="u-padding-20">
		<u-form :model="form" ref="uForm" label-width="auto">
			...
			<u-form-item label="开始时间" prop="crUseStartDate">
				<u-input v-model="form.crUseStartDate" placeholder="请选择开始时间" type="select" @click="startShow = true" />
				<u-picker
					mode="time"
					v-model="startShow"
					:params="dateParams"
					title="开始时间"
					@confirm="e => (form.crUseStartDate = e.year + '-' + e.month + '-' + e.day + ' ' + e.hour +':00:00')"
				></u-picker>
			</u-form-item>
			<u-form-item label="截止时间" prop="crUseEndDate">
				<u-input v-model="form.crUseEndDate" placeholder="请选择截止时间" type="select" @click="endShow = true" />
				<u-picker
					mode="time"
					v-model="endShow"
					:params="dateParams"
					title="截止日期"
					@confirm="e => (form.crUseEndDate = e.year + '-' + e.month + '-' + e.day + ' ' + e.hour +':00:00')"
				></u-picker>
			</u-form-item>
		</u-form>
		...
		submit(e) {
			...
			this.$refs.uForm.validate(valid => {
				if (valid) {
					...
					uni.showModal({
						title: '修改确认',
						content: `是否确认修改商品：${formD.couponTitle + '的' + that.typeList[formD.couponType].text}`,
						success: res => {
							if (res.confirm) {
